...
支持 1Hz~100kHz 频率输出、FG 信号采集与 RPM 显示、VSP 模拟电压输出 (0~5V, 0.01V 步进)、自动测试模式 (PWM/继电器 ON/OFF 循环 + 数据记录 + CSV 导出)，以及 Flappy Bird 小游戏彩蛋。附带 **Python PyQt6 桌面模拟器**，可在 PC 上仿真全部操作界面并通过串口连接实物。
...
双击旋转编码器循环切换：**PWM-FG → FG → CH1 → CH2 → VSP → TEST → GAME**
...
| **GAME** | Flappy Bird 小游戏彩蛋，旋钮点击控制小鸟跳跃 |
...
│   ├── HARDWARE/APP/       应用层核心代码 (7 模式菜单/协议/UI/驱动)
